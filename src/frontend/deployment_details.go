...
func loadDeploymentDetails() {
	...
	// Use environment variables for deployment details instead of cloud-specific metadata
	podCluster := os.Getenv("CLUSTER_NAME")
	if podCluster == "" {
		podCluster = "local-cluster"
	}

	podZone := os.Getenv("DEPLOYMENT_ZONE")
	if podZone == "" {
		podZone = "local-zone"
	}
